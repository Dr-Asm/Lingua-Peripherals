...
|--------|---------|-------------|
| isTapePresent() | oolean | Whether a cassette tape is inserted |
| getTapeID() | 
umber or 
il | The unique ID of the inserted tape |
| getTapeLabel() | string or 
il | The label of the inserted tape |
...
| dataSize() | 
umber | Current data file size in bytes |
| dataSizeLimit() | 
umber | Maximum data size limit in bytes (configurable) |
| open(mode) | file handle | Opens the tape data file (modes: "r", "w", "a", "rb", "wb", "ab", "r+", "w+", "a+") |
| close() | — | Force-closes any open file handle |
| 
eset() | — | Closes handle and clears all data |

The file handle returned by open() supports 
ead(), write(), writeLine(), seek(), and close() — similar to Lua's io library.
